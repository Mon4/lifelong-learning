...
from my_rm.utils.data_loader import cutmix_data, ImageDataset
...
class RM(Finetune):
    ...
    def train(self, cur_iter, n_epoch, batch_size, n_worker, n_passes=0):
        ...
        print(f"Streamed samples: {len(self.streamed_list)}")
        print(f"In-memory samples: {len(self.memory_list)}")
        print(f"Train samples: {len(train_list)+len(self.memory_list)}")
        ...
        for epoch in range(n_epoch):
            ...
            print(f"task{cur_iter}/test/loss", eval_dict["avg_loss"], epoch)
            print(f"task{cur_iter}/test/acc", eval_dict["avg_acc"], epoch)
            print(f"task{cur_iter}/train/lr", self.optimizer.param_groups[0]["lr"], epoch)
            print(f"Task {cur_iter} | Epoch {epoch+1}/{n_epoch} | train_loss {train_loss:.4f} | train_acc {train_acc:.4f} | "
